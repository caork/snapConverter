...
import com.snapconverter.engine.VmafUnavailableException
...
enum class QualityTargetMetric { SSIM, VMAF }

/**
 * Encodes a few short windows at candidate bitrates and picks the lowest
 * bitrate whose SSIM or VMAF still meets the target. Full-file encode stays one pass.
 */
class QualityTargetCalibrator(
    ...
    fun calibrate(
        ...
        metric: QualityTargetMetric,
        target: Double,
        progress: EncodeProgressListener?,
    ): SsimLadder.Result {
        if (metric == QualityTargetMetric.VMAF && !VmafNative.available) {
            throw VmafUnavailableException()
        }
        val windows = SsimLadder.windows(source.durationUs)
        val hi = upperBoundBps(source)
        val lo = 200_000
        val label = metricLabel(metric)
        ScLog.i("$label calibrate target=$target windows=${windows.size} lo=$lo hi=$hi")
        var step = 0
        val result = SsimLadder.minBitrateForScore(
            loBps = lo,
            hiBps = hi,
            target = target,
            targetMin = if (metric == QualityTargetMetric.VMAF) 0.0 else 0.50,
            targetMax = if (metric == QualityTargetMetric.VMAF) 100.0 else 0.999,
        ) { bitrate ->
            step++
            progress?.onProgress(
                EncodeProgress(
                    ratio = (0.02f + 0.14f * (step / 7f)).coerceIn(0.02f, 0.16f),
                    message = "正在标定 $label（$step）…",
                ),
            )
            measureWindows(input, source, decoder, encoder, request, windows, bitrate, metric)
        }
        ScLog.i(
            "$label calibrate done bitrate=${result.bitrateBps} score=${result.score} " +
                "iters=${result.iterations} met=${result.metTarget}",
        )
        val scoreText = if (metric == QualityTargetMetric.VMAF) {
        ...
        progress?.onProgress(
            EncodeProgress(
                ratio = 0.18f,
                message = "$label 标定 $scoreText @ ${result.bitrateBps / 1000} kbps",
            ),
        )
        return result
    }
    ...
    private fun measureWindows(
        ...
        val scores = windows.mapNotNull { window ->
            measureOne(input, source, decoder, encoder, request, window, bitrateBps, metric)
        }
    ...
    private fun measureOne(
        ...
        val tmp = File.createTempFile("sc-metric-", ".mp4", context.cacheDir)
        try {
            ...
            val report = quality.compare(
                ...
                includeVmaf = metric == QualityTargetMetric.VMAF,
            )
            val score = when (metric) {
                QualityTargetMetric.SSIM -> report?.ssim
                QualityTargetMetric.VMAF -> report?.vmaf
            }
            ScLog.i(
                "${metricLabel(metric)} probe ${window.startUs / 1000}ms @ ${bitrateBps / 1000}kbps " +
                    "score=$score samples=${report?.samples}",
            )
            return score
        } catch (t: Throwable) {
            ScLog.w("${metricLabel(metric)} probe failed at ${bitrateBps / 1000} kbps", t)
            return null
        } finally {
            tmp.delete()
        }
    ...
    private fun metricLabel(metric: QualityTargetMetric): String = when (metric) {
        QualityTargetMetric.SSIM -> "SSIM"
        QualityTargetMetric.VMAF -> "VMAF"
    }
}
